---
layout: default
title: Home
---

# 🎄 Advent of Code 2025

My solutions for [Advent of Code 2025](https://adventofcode.com/2025) implemented in Java 21 with Bazel.

[![Build status](https://badge.buildkite.com/2f9c44cb4ac88cebbae4916853e755ee088a81e0c1c9adce7d.svg)](https://buildkite.com/bobcat/advent-of-code-2025)

## 🌟 Solutions

| Day | Challenge | Part 1 | Part 2 | Solution Documentation |
|-----|-----------|--------|--------|------------------------|
| 1   | [Secret Entrance](https://adventofcode.com/2025/day/1) | 1145 | 6561 | [📖 Read Solution](docs/day01.html) |
| 2   | [Gift Shop](https://adventofcode.com/2025/day/2) | 54234399924 | 70187097315 | [📖 Read Solution](docs/day02.html) |
| 3   | [Lobby](https://adventofcode.com/2025/day/3) | 17330 | 171518260283767 | [📖 Read Solution](docs/day03.html) |
| 4   | [Printing Department](https://adventofcode.com/2025/day/4) | 1553 | 8442 | [📖 Read Solution](docs/day04.html) |
| 5   | [Cafeteria](https://adventofcode.com/2025/day/5) | 694 | 352716206375547 | [📖 Read Solution](docs/day05.html) |
| 6   | [Trash Compactor](https://adventofcode.com/2025/day/6) | 5552221122013 | 11371597126232 | [📖 Read Solution](docs/day06.html) |
| 7   | [Laboratories](https://adventofcode.com/2025/day/7) | 1570 | 15118009521693 | [📖 Read Solution](docs/day07.html) |
| 8   | [Playground](https://adventofcode.com/2025/day/8) | 96672 | 22517595 | [📖 Read Solution](docs/day08.html) |
| 9   | [Movie Theater](https://adventofcode.com/2025/day/9) | 4781546175 | 1573359081 | [📖 Read Solution](docs/day09.html) |
| 10  | [Factory](https://adventofcode.com/2025/day/10) | 375 | 15377 | [📖 Read Solution](docs/day10.html) |
| 11  | [Reactor](https://adventofcode.com/2025/day/11) | 674 | 438314708837664 | [📖 Read Solution](docs/day11.html) |
| 12  | [Christmas Tree Farm](https://adventofcode.com/2025/day/12) | 577 | N/A | [📖 Read Solution](docs/day12.html) |

⭐ **23 puzzles solved!** Current progress: **23/23 ⭐** (Complete!)

---

## 📚 About This Project

This repository contains my solutions for Advent of Code 2025, featuring:

- **Clean Java 21 implementations** with modern language features
- **Comprehensive test coverage** including example tests and regression tests
- **Detailed solution documentation** explaining algorithms and approaches
- **Bazel build system** for fast, reproducible builds
- **CI/CD pipeline** with automated testing and result annotations

## 🔍 Explore Solutions

Each day's solution includes:
- Complete puzzle description from Advent of Code
- Algorithm explanation with complexity analysis
- Code walkthrough with detailed comments
- Example test cases and actual results

Click on the "📖 Read Solution" links above to dive into each day's detailed explanation!

## ⚡ Performance Analysis

Curious about how fast these solutions run? Check out the comprehensive [Performance Analysis](docs/performance.html) with detailed benchmarks for all solutions, including:
- Execution time statistics (min, max, average, median)
- Performance comparisons and visualizations
- Solution categorization by speed

## 💻 Source Code

Visit the [GitHub repository](https://github.com/stiksy/advent-of-code-2025) to see the full source code, including:
- Java solution implementations
- JUnit test suites
- Bazel BUILD files
- CI/CD configuration

## 📖 Development Guide

For information about the development workflow, testing requirements, and how to add new solutions, see the [development guide](https://github.com/stiksy/advent-of-code-2025/blob/main/CLAUDE.md).

---

## 🙏 Acknowledgments

- [Advent of Code](https://adventofcode.com/) by Eric Wastl
- Solutions developed with assistance from [Claude Code](https://claude.com/code)
